    ...
    if 'User_email' in request.session.keys():
        ...
        if user_model.is_approved==True:
            if request.POST:
                ...
                if form.is_valid():
                    form.save()
                    list_model = ListingAppliances.objects.latest('id')
...
def Update_Owner_Property(request,id):
    if 'Owneremail' in request.session.keys():
        obj = ListingAppliances.objects.get(id=id)
        ...
        if request.POST:
            ...
            obj.appliance_type = request.POST['appliance_type']
            
            OC = request.POST.get('OriginalCharger')
            if OC == "on":
                obj.OriginalCharger = True
            else:
                obj.OriginalCharger = False
            bx = request.POST.get('box')
            if bx == 'on':
                obj.box = True
            else:
                obj.box = False
            OB= request.POST.get('OriginalBill')
            if OB== "on":
                obj.OriginalBill = True
            else:
                obj.OriginalBill = False
            WR = request.POST.get('warranty')
            if WR == "on":
                obj.warranty = True
            else:
                obj.warranty = False
...
def AllListView(request):
    if 'User_email' in request.session.keys():
        user_model = UserRegistration.objects.get(email_id=request.session['User_email'])
        list_model = ListingAppliances.objects.all().filter(available=True)
        if request.GET:
            try:
            ...
            if q != None and request.GET['search_data'] != None:
                appliancetype = ListingAppliances.objects.all().filter(available=True)
                appliance = ListingAppliances.objects.filter(Q(title__icontains=q) | Q(address__icontains=q) | Q(description__icontains=q) | Q(available__icontains=True))
                print("Q Collect")
                return render(request, 'listing.html', {'appliance1':appliance,'appliance':appliancetype})
            else: 
                search = request.GET.get('text')
                pricefilter = request.GET.get('pricefilter')
                appliancetype = ListingAppliances.objects.all().filter(available=True)
                print("Q other Data")
                appliance = request.GET.get('appliance_type')
                if appliance != '':
                    list_model = ListingAppliances.objects.filter(appliance_type=appliance).order_by('price') & ListingAppliances.objects.filter(available=True)
                    user_filter = FilterDemo(request.GET, queryset=list_model)

                if pricefilter == 'low':
                    list_model = ListingAppliances.objects.all().filter(available=True).order_by('price')
                    user_filter = FilterDemo(request.GET, queryset=list_model)
                    ...
                    list_model = ListingAppliances.objects.all().filter(available=True)
                    user_filter = FilterDemo(request.GET, queryset=list_model)
                return render(request, 'listing.html', {'all_list': user_filter,'appliance':appliancetype,'users_data':user_model})
        else:
            print("Q POST")
            appliancetype = ListingAppliances.objects.all().filter(available=True)
            print("Q POst DAta")
        ...
        if request.GET:
            ...
            if q != None and request.GET['search_data'] != None:
                appliancetype = ListingAppliances.objects.all().filter(available=True)
                appliance = ListingAppliances.objects.filter(Q(title__icontains=q)  | Q(description__icontains=q) | Q(available__icontains=True))
                print("Q Collect")
                return render(request, 'listing.html', {'appliance1':appliance,'appliance':appliancetype})
            else: 
                search = request.GET.get('text')
                pricefilter = request.GET.get('pricefilter')
                appliancetype =ListingAppliances.objects.all()
                print("Q other Data")
                appliance = request.GET.get('appliance_type')
                if appliance != '':
                    list_model = ListingAppliances.objects.filter(appliance_type=appliance).order_by('price') & ListingAppliances.objects.filter(available=True)
            ...
            list_model = ListingAppliances.objects.all().filter(available=True)
            user_filter = FilterDemo(request.GET, queryset=list_model)
    return render(request, 'listing.html', {'all_list': user_filter,'appliance':appliancetype})
...
def OwnerAllListView(request):
    if 'Owneremail' in request.session.keys():
        ...
        if request.GET:
            ...
            if q != None and request.GET['search_data'] != None:
                appliancetype = ListingAppliances.objects.all()
                appliance = ListingAppliances.objects.filter(Q(title__icontains=q) |  Q(description__icontains=q))
                print("Q Collect")
                return render(request, 'Ownerlisting.html', {'appliance':appliance,'appliance':appliance,'owner_data':user})
            else: 
                search = request.GET.get('text')
                pricefilter = request.GET.get('pricefilter')
                appliancetype = ListingAppliances.objects.all()
                ...
                appliance = request.GET.get('appliance_type')
                if appliance != '':
                    list_model = ListingAppliances.objects.filter(appliance_type=appliance).order_by('price')
                    user_filter = FilterDemo(request.GET, queryset=list_model)

                if pricefilter == 'low':
                    list_model = ListingAppliances.objects.all().order_by('price')
                    user_filter = FilterDemo(request.GET, queryset=list_model)
                elif pricefilter == 'high':
                    list_model = ListingAppliances.objects.all().order_by('-price')
                    user_filter = FilterDemo(request.GET, queryset=list_model)
                else:
                    list_model = ListingAppliances.objects.all().filter(verified=True)
                    user_filter = FilterDemo(request.GET, queryset=list_model)
                return render(request, 'Ownerlisting.html', {'all_list': user_filter,'appliance':appliancetype,'owner_data':user})
        else:
            print("Q POST")
            appliancetype = ListingAppliances.objects.all()
            print("Q POst DAta")
            list_model = ListingAppliances.objects.all().filter(available=True)
            user_filter = FilterDemo(request.GET, queryset=list_model)
        list_model = ListingAppliances.objects.all()
        user_filter = FilterDemo(request.GET, queryset=list_model)
        return render(request, 'Ownerlisting.html', {'all_list': user_filter,'appliance':appliancetype,'owner_data':user})
...
def UserIndexPage(request):
    if 'User_email' in request.session.keys():
        ...
        prod = ListingAppliances.objects.all().filter(available=True)
        ...
        for i in prod:
            ...
            print("------------------")
            print(i.id)
            fda = ProperFeedback.objects.filter(Appliance_name=i)
            tot_pos = 0.0
            tot_ret = 0.0
            count = 0
            for j in fda:
        ...
        list_model = ListingAppliances.objects.all().order_by('-id')[:3]
...
def ProfileView(request, id):
    if 'User_email' in request.session.keys():
        user=UserRegistration.objects.get(email_id=request.session['User_email'])
        model = ListingAppliances.objects.get(id=id)
        q = str(model.country)
        model.view_count += 1
        model.save()
        dataset = ''
        try:
            dataset = ListingAppliances.objects.filter(Q(country__icontains=q)| Q(address__icontains=q))[0:4]
        except:
            dataset = ListingAppliances.objects.all()[0:4]
        feeds = ProperFeedback.objects.filter(Appliance_name=model)
        try:
            User_feed = ProperFeedback.objects.get(Appliance_name=model,cust_data=user)
            if request.POST:
                User_feed.Appliance_name = model
...
def Owner_myproperty(request):
    if 'Owneremail' in request.session.keys():
        user_model = OwnerDetails.objects.get(Owneremail=request.session['Owneremail'])
        all_list = ListingAppliances.objects.filter(email_id=request.session['Owneremail'])
...
def MyPropertyView(request):
    if 'User_email' in request.session.keys():
        user=UserRegistration.objects.get(email_id=request.session['User_email'])
        all_list = ListingAppliances.objects.filter(email_id=request.session['User_email'])
...
def prop_booking(request,id):
    if 'User_email' in request.session.keys():
        user=UserRegistration.objects.get(email_id=request.session['User_email'])
        prod=ListingAppliances.objects.get(id=id)
...
def UserBooks(request):
    if 'User_email' in request.session.keys():
        user=UserRegistration.objects.get(email_id=request.session['User_email'])
        prod = Orders.objects.filter(user_id=user)
...
def alldata(request):
    if 'Owneremail' in request.session.keys():
        ...
        book=Orders.objects.all().filter(owner_id=owner)
...
def userbuy(request):
    if 'Owneremail' in request.session.keys():
        owner=OwnerDetails.objects.get(Owneremail=request.session['Owneremail'])
        mod=Orders.objects.filter(owner_id=owner)
...
def paymenthandler(request):
    ...
    if request.method == "POST":
        try:
            ...
            orderModel.appliance_name = request.session['appliance_name']
            ...
            orderModel.save()
            d=Orders.objects.get(id=request.session['id'])
            d.payment=True
            d.save()
...
def ordertable(request):
    if 'User_email' in request.session.keys():
        user=UserRegistration.objects.get(email_id=request.session['User_email'])
        order=paymentdetails.objects.filter(user=user.full_name)
...
def ownerordertable(request):
    if 'Owneremail' in request.session.keys():
        owner=OwnerDetails.objects.get(Owneremail=request.session['Owneremail'])
        order=paymentdetails.objects.filter(owner_id=owner.pk)
